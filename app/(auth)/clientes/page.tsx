    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        <button
          ...
          className={`flex-1 min-w-fit h-12 px-4 rounded-lg font-medium text-sm border transition-colors ${toggleActivo}`}
        >
          Todos
        </button>
        <button
          onClick={() => setFiltro('con_deuda')}
          className={`flex-1 min-w-fit h-12 px-4 rounded-lg font-medium text-sm border transition-colors ${toggleDeuda}`}
        >
          Con deuda
        </button>
        <button
          onClick={() => setFiltro('al_dia')}
          className={`flex-1 min-w-fit h-12 px-4 rounded-lg font-medium text-sm border transition-colors ${toggleAlDia}`}
        >
          Al día
        </button>
        <button
          onClick={() => setFiltro('bloqueados')}
          className={`flex-1 min-w-fit h-12 px-4 rounded-lg font-medium text-sm border transition-colors ${toggleBloqueados}`}
        >
          Bloqueados
        </button>
      </div>
      ...
      {error && (
        <Card className="p-4 bg-red-50 border border-red-200">
          ...
          <Button
            ...
            onClick={() => cargarClientes(buscar, filtro)}
          >
            Reintentar
          </Button>
        </Card>
